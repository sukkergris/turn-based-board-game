...
public class WorldBuilder
{
    ...
    public World Build(uint x, uint y)
    {
        ...
        var coordinates = xaxis
            .SelectMany(x =>
                yaxis.Select(y => new ClosedCoordinates(
                    XCoordinate.Create((uint)x),
                    YCoordinate.Create((uint)y)
                ))
            )
            .ToArray();

        _world.Squares = coordinates.ToDictionary(
            coordinate => coordinate,
            coordinate => new SquareBuilder().Build(coordinate)
        );
